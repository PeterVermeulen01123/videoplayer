...
function changeVolume(e) {
    ...
    video.volume = volume;
   // console.log(volume);
    //change audio icon depending on volume level
    volumeIcon.className = '';
    if(volume > 0.7) {
        volumeIcon.classList.add('fas', 'fa-volume-up');
    }else if(volume<0.9 && volume>0) {
...
function toggleMute() {
    ...
    if(video.volume) {
        ...
        volumeIcon.setAttribute('title', 'Unmite');
...
function toggleFullscreen() {
    if(document.fullscreenElement){//document is full screen
        closeFullscreen();
        video.classList.remove('video-fullscreen');
    ...
    }else{//document is not full screen
...
document.addEventListener('fullscreenchange', (event) => {
     if(!document.fullscreenElement){ //check if it is full screen now
